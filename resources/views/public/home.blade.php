...
@section('content')
    ...
    <section class="section-gap services-area" id="services-list">
      <div class="container">
        ...
        <div class="row">
          ...
          @empty
            <div class="col-12"><p class="text-center">Belum ada layanan tersedia.</p></div>
          @endforelse
        </div>
        <div class="text-center mt-2">
          <a href="{{ route('public.services') }}" class="primary-btn d-inline-flex align-items-center">Lihat Semua Layanan <span class="lnr lnr-arrow-right ml-2"></span></a>
        </div>
      </div>
    </section>
            ...
            <div class="col-lg-4 col-md-6 mb-4">
              <div class="signle-service border rounded overflow-hidden h-100">
                <a href="{{ route('public.portfolio-details', $project) }}" class="d-block">
                  <img src="{{ $thumb ? asset('storage/'.$thumb) : asset('public_template/img/s1.jpg') }}" class="img-fluid w-100" alt="{{ $project->project_title }}">
                </a>
                <figcaption class="p-3">
                  ...
                  <a href="{{ route('public.portfolio-details', $project) }}" class="primary-btn d-inline-flex align-items-center">Lihat Detail<span class="lnr lnr-arrow-right"></span></a>
                </figcaption>
              </div>
            </div>
          @empty
            <div class="col-12"><p class="text-center">Belum ada project ditampilkan.</p></div>
          @endforelse
        </div>
        <div class="text-center mt-2">
          <a href="{{ route('public.portfolio') }}" class="primary-btn d-inline-flex align-items-center">Lihat Semua Portfolio <span class="lnr lnr-arrow-right ml-2"></span></a>
        </div>
      </div>
    </section>
            ...
            <div class="owl-carousel" id="testimonials-carousel">
              @forelse($testimonials as $t)
                <div class="single-testimonial text-center p-4">
                  ...
                  <img src="{{ $avatar }}" class="img-fluid rounded-circle mb-3" style="width:80px;height:80px;object-fit:cover;" alt="{{ $t->client_name }}">
                  <p class="mb-2">“{{ $t->testimonial_text }}”</p>
                  <h6 class="text-uppercase mb-0">{{ $t->client_name }}</h6>
                </div>
              @empty
                <div class="text-center"><p>Belum ada testimoni.</p></div>
              @endforelse
            </div>
          </div>
        </div>
    ...
    <section class="section-gap" id="latest-blog">
      <div class="container">
        ...
        <div class="text-center mt-2">
          <a href="{{ route('public.blog') }}" class="primary-btn d-inline-flex align-items-center">Lihat Semua Artikel <span class="lnr lnr-arrow-right ml-2"></span></a>
        </div>
      </div>
    </section>
